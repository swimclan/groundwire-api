...
module.exports.authenticatedUser = function(req) {
  if (!_.isUndefined(req.headers.authorization) || !_.isUndefined(req.cookies.rh_sid)) {
    let authSplit = req.headers.authorization ? req.headers.authorization.split(' ') : ['Bearer', req.cookies.rh_sid];
    let authorization = {};
...
module.exports.loggedIn = function(req, res) {
	...
	if (_.isUndefined(session)) {
		return this.sendJSONResponse(401, res, {error: 'No authorized session detected'});
  }
  return session;
}
