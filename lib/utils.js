...
module.exports.secure = function(req, res) {
  if (!validAPIKey(req)) return this.sendJSONResponse(401, res, { error: "Unauthorized: Invalid or no API key provided" });
}

module.exports.findQueuedStop = function(orders, inst) {
  var self = this;
  for (var i=0; i<orders.length; i++) {
    if ((orders[i].state === 'queued') && (orders[i].trigger === 'stop') && (orders[i].instrument === self.buildInstrumentUrl(inst))) {
      return orders[i];
    }
  };
  return false;
