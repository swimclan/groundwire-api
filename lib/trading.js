	...
	return new Promise(function(resolve, reject) {
		var rh = Robinhood(credentials, function() {
			rh.orders(function(err, response, body) {
				...
				console.log(foundQueuedStop);
				if (foundQueuedStop) return resolve(foundQueuedStop);
				async.whilst(
					function() { return !foundQueuedStop && nextCursorUrl !== null  },
					function(callback) {
						rh.url(nextCursorUrl, function(err, response, body) {
							if (err) {
								callback(err, null);
							}
							orders = body.results;
							nextCursorUrl = body.next;
							foundQueuedStop = utils.findQueuedStop(orders, instrumentId);
							if (foundQueuedStop) callback(null, foundQueuedStop);
						});
					},
					function(err, foundStop) {
						if (err) {
							console.log(err);
							reject(err);
						}
