...
module.exports.expireUser = function(rh) {
	logger.log('User expire', 'About to logout user and expire token');
	let old_token = rh.auth_token();
	return new Promise(function(resolve, reject) {
		rh.expire_token(function(err, response) {
			if (err) {
				logger.log('ERROR!', 'Failed to expire user with error', {error: err});
				reject(err);
			}
			logger.log('User expire', 'Successfully expired user token for:', {token: old_token});
			resolve({token: old_token});
		});
	});
}
...
module.exports.findOrdersByInstrument = function(rh, state, instrumentId) {
	...
	return new Promise(function(resolve, reject) {
		rh.orders(function(err, response, body) {
			if (err) {
				logger.log('ERROR!', 'Failed to fetch orders with error (264):', {error: err});
				reject(err);
			}
			var retOrders = body.results;
			var nextCursorUrl = body.next;
			async.whilst(
				function() { return (nextCursorUrl !== null) && (!_.isUndefined(nextCursorUrl)) },
				function(callback) {
				...
				function(err, ret) {
					if (err) {
						logger.log('ERROR!', 'Failed to fetch orders with error (284):', {error: err});
						reject(err);
					}
					var instUrl = utils.buildInstrumentUrl(instrumentId);
					var queued;
...
module.exports.findRecentOrdersByInstrument = function(rh, state, instrument, date) {
	let instrumentUrl = utils.buildInstrumentUrl(instrument);
	logger.log('Recent orders', `About to fetch ${state} orders since ${date} for instrument: ${instrumentUrl}:`, {instrument: instrumentUrl, date: date, state: state});
	return new Promise(function(resolve, reject) {
		let options = {updated_at: date, instrument: utils.buildInstrumentUrl(instrument)};
		rh.orders(options, function(err, response, body) {
			if (err) {
				logger.log('ERROR!', 'Failed to fetch recent orders (311):', {error: err});
				reject(err);
			}
			var retOrders = body.results;
			var nextCursorUrl = body.next;
			if (!nextCursorUrl) {
				logger.log('Recent orders', `Successfully fetched ${retOrders.length} recent orders`, retOrders);
				return resolve(utils.filterOrdersByState(retOrders, state));
			}
			async.whilst(
				function() { return (nextCursorUrl !== null) && (!_.isUndefined(nextCursorUrl)) },
				function(callback) {
				...
				function(err, ret) {
					if (err) {
						logger.log('ERROR!', 'Failed to fetch recent orders with error (330):', {error: err});
						reject(err);
					}
					var queued;
					if (ret) {
						queued = utils.filterOrdersByState(ret, state);
						logger.log('Orders', `Successfully fetched ${queued.length} recent orders`, queued);
					} else {
						logger.log('WARNING!', 'No orders were found or something failed on fetch and got undefined/empty set');
						queued = [];
					}
...
module.exports.getYahooPrice = function(ticker) {
	...
	var quote_params = {
		formatted: config.get('yahoo.url.formatted'),
		lang: config.get('yahoo.url.lang'),
		region: config.get('yahoo.url.region'),
		symbols: ticker
	};
	var url = quote_root + queryString.stringify(quote_params);
	logger.log('Yahoo price', 'About to fetch Yahoo price data:', {url: url});
	return new Promise(function(resolve, reject) {
		request({url: url}, function(err, response, body) {
			if (err) {
				logger.log('ERROR!', 'Failed to fetch yahoo price data:', {error: err});
				reject(err);
			}
			try {
				logger.log('Yahoo price', 'Successfully retrieved price data body, now parsing...', {response: body});
				var result = JSON.parse(body).quoteResponse.result[0];
				if (_.has(result, 'regularMarketPrice')) {
					var regularMarketPrice = result.regularMarketPrice;
				} else {
					logger.log('WARNING!', `Invalid or non-existent instrument: ${ticker}`, {warning: 'Invalid yahoo request'});
					return resolve({warning: 'Invalid or non-existent instrument'})
				}
				logger.log('Yahoo price', 'Successfully fetched Yahoo price data:', {price: regularMarketPrice.raw});
				resolve({price: regularMarketPrice.raw});
			} catch (e) {
				logger.log('ERROR!', 'Failed to fetch Yahoo price data (parsing):', {error: e});
				reject(e);
			}
		});
	});
